 ...
 */
package org.ncbo.resource_access_tools.resource.nif.genenetwork;
...
public class GENETAccessTool extends AbstractNifResourceAccessTool {
    ...
    private static final String GENET_ELT_URL = "http://www.genenetwork.org/webqtl/main.py?FormID=showDatabase&database=";
    private static final String[] GENET_ITEMKEYS = {"Gene_Name", "Chromosome_location", "Tissue", "Organism", "Originating_database"};
    private static final Double[] GENET_WEIGHTS = {1.0, 0.7, 0.9, 0.8, 0.5};
    ...
    private static final Structure GENET_STRUCTURE = new Structure(GENET_ITEMKEYS, GENET_RESOURCEID, GENET_WEIGHTS, GENET_ONTOIDS);
    // Constant
    private static final String nifId = "nif-0000-00380-1";
    ...
    public GENETAccessTool() {
        ...
        Map<String, String> localOntologyIDMap = createLocalOntologyIDMap(GENET_STRUCTURE);
    }
    ...
    public String mainContextDescriptor() {
        String GENET_MAIN_ITEMKEY = "Gene_Name";
                    ...
                    for (int i = 0; i < rows.getLength(); i++) {
                        ...
                        for (int j = 0; j < row.getChildNodes().getLength(); j++) {
                            ...
                            if (name.equalsIgnoreCase(GENET_Gene_Name)) { //Gene_Name & localElementId
                                ...
                                localElementId = value.substring(value.indexOf(GENET_ELT_URL) + GENET_ELT_URL.length(), value.indexOf(endTag));
                        ...
                        if (allElementsInET.contains(localElementId)) {
                            ...
                            //adding chrmosome location field values to geneName.
                            String chr = elementAttributes.get(Structure.generateContextName(GENET_RESOURCEID, GENET_ITEMKEYS[1]));
                            String geneName = elementAttributes.get(Structure.generateContextName(GENET_RESOURCEID, GENET_ITEMKEYS[0]));
                            if (chr.length() > 0 && geneName.length() > 0) {
                                geneName += BLANK_SPACE + chr;
                            }
