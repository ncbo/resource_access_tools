 ...
 */
package org.ncbo.resource_access_tools.resource.nif.ctdgene;
 ...
 * @author r.malviya
 */
public class CTDGAccessTool extends AbstractNifResourceAccessTool {

	private static final String URL = "http://ctdbase.org/";
	private static final String NAME = "CTD Gene (via NIF)";
	private static final String RESOURCEID = "CTDG";
	...
	private static final String[] ITEMKEYS = { "GeneSymbol", "GeneName",
			"AltGeneIDs", "Synonyms", "BioGRIDIDs", "PharmGKBIDs", "UniProtIDs" };
	private static final Double[] WEIGHTS = { 1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4 };
	...
	public void getAllElements() {
		...
		try {
			...
			URL csvFile = new URL(
					"http://ctdbase.org/reports/CTD_genes.tsv.gz");
						...
						try {
							...
							logger.error(
									"** PROBLEM ** Problem with id "
											...
											+ " when populating the OBR_CTDG_ET table.",
									e);
						}

					}
					logger.info(nbElement
							+ " elements added to the OBR_CTDG_ET table.");
				}
			}
